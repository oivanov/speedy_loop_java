...
public class Routes {
  ...
  public int numberOfStopsBetweenTowns(Town origin, Town destination, int limit){
    ...
    public int numberOfRoutsBetweenTownsExactStopCount(Town origin, Town destination, int limit){
        return findRoutesExactStopCount(origin, destination, 0,limit);
    }
  ...
  private int findRoutes(Town origin, Town dest, int depth, int limit) {
    ...
    private int findRoutesExactStopCount(Town origin, Town dest, int depth, int limit) {
        int routes = 0;
        if(this.routingTable.containsKey(origin) && this.routingTable.containsKey(dest)) {
            if(depth == limit){ return 0;}
            depth++;
